...
def carr_wu_seasonal_decay_kappa_omega_rho(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    eta1_star, eta2_star, eta3_star, eta4_star, kappa1, omega1_star, nu_star, rho1_star = f.flatten()[:8]
    deltas = f[8:]
...
class CarrWuSeasonalDecayKappaOmegaRho:
    def __init__(self, mean:str = 'carr_wu_s1_decay_kappa_omega_rho'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_decay_kappa_omega_rho
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay_kappa_omega_rho':2, 'carr_wu_s2_decay_kappa_omega_rho':4,
                                 'carr_wu_s3_decay_kappa_omega_rho':6, 'carr_wu_s4_decay_kappa_omega_rho':8})
        self.k_fxd = 8 + self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
