...
def carr_wu_disprop2_seasonal_time_space(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    t = T - tau
    kappa1, omega1_star, nu_star, rho_star, eta1_tilde, eta2_tilde, d1, d2 = f.flatten()[:8]
    S2 = d1* np.sin(2*np.pi *t ) + d2*np.cos(2*np.pi *t )
    ...
    kappa = S2 + kappa1 * exp_eta1_tau + S * exp_eta2_tau
...
class CarrWuDisProp2SeasonalTimeSpace:
    def __init__(self, mean:str = 'carr_wu_disprop2_s1_time_space'):
        self.mean = mean
        self.Zf = carr_wu_disprop2_seasonal_time_space
        self.J = None #jac_carr_wu_disprop_seasonal_fxd
        self._dict_means = dict({'carr_wu_disprop2_s1_time_space':2, 'carr_wu_disprop2_s2_time_space':4,
                                 'carr_wu_disprop2_s3_time_space':6, 'carr_wu_disprop2_s4_time_space':8})
