...
def carr_wu_seasonal(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega_star, nu_star, rho_star = f.flatten()[:4]
    deltas = f[4:]
...
class CarrWuSeasonal:
    def __init__(self, mean:str = 'carr_wu_s1'):
        self.mean = mean
        self.Zf = carr_wu_seasonal
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1':2, 'carr_wu_s2':4,'carr_wu_s3':6, 'carr_wu_s4':8})
        self.k_fxd = 4 + self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
