...
def carr_wu_seasonal_decay(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega_star, nu_star, rho_star, eta_star = f.flatten()[:5]
    deltas = f[5:]
    omega, nu, rho, gamma = map_moments(omega_star, nu_star, rho_star)
    S = seasonal_effect(deltas, T)
    kappa = kappa1 + S * np.exp(eta_star * tau)
...
class CarrWuSeasonalDecay:
    def __init__(self, mean:str = 'carr_wu_s1_decay'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_decay
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay':2, 'carr_wu_s2_decay':4,'carr_wu_s3_decay':6, 'carr_wu_s4_decay':8})
        self.k_fxd = 5 + self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
