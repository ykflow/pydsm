...
def carr_wu_seasonal_corr(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega_star, nu_star, rho1_star, eta1_tilde, eta2_tilde = f.flatten()[:6]
    omega, nu, rho1, gamma = map_moments(omega_star, nu_star, rho1_star)
    deltas = f[6:]
    exp_eta1_tau = np.exp(eta1_tilde*tau)
    exp_eta2_tau = np.exp(eta2_tilde*tau)
    S = seasonal_effect(deltas, T)
    kappa = kappa1 + S * exp_eta2_tau
    rho = rho1 * exp_eta1_tau
...
class CarrWuSeasonalRho:
    def __init__(self, mean:str = 'carr_wu_s1_corr'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_corr
        self.J = None #jac_carr_wu_disprop_seasonal_fxd
        self._dict_means = dict({'carr_wu_s1_corr':2, 'carr_wu_s2_corr':4,
                                 'carr_wu_s3_corr':6, 'carr_wu_s4_corr':8})
