...
def carr_wu_seasonal_decay_omega_rho_nu(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega_star, nu1_star, rho1_star, eta1_star, eta2_star, eta3_star, eta4_star = f.flatten()[:8]
    deltas = f[8:]
    omega1, nu1, rho1, gamma = map_moments(omega_star, nu1_star, rho1_star)
    S = seasonal_effect(deltas, T)
    kappa = kappa1 + S * np.exp(eta1_star * tau)
    omega = omega1 * np.exp(eta2_star * tau)
    rho = rho1 * np.exp(eta3_star * tau)
...
class CarrWuSeasonalDecayOmegaRhoNu:
    def __init__(self, mean:str = 'carr_wu_s1_decay_omega_rho_nu'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_decay_omega_rho_nu
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay_omega_rho_nu':2, 'carr_wu_s2_decay_omega_rho_nu':4,
                                 'carr_wu_s3_decay_omega_rho_nu':6, 'carr_wu_s4_decay_omega_rho_nu':8})
