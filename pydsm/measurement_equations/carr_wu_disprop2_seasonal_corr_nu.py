...
def carr_wu_disprop2_seasonal_corr_nu(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega1_star, nu1_star, rho1_star, eta1_tilde, eta2_tilde, eta3_tilde, eta4_tilde = f.flatten()[:8]
    omega1, nu1, rho1, gamma = map_moments(omega1_star, nu1_star, rho1_star)
    deltas = f[8:]
    exp_eta1_tau = np.exp(eta1_tilde * tau)
    exp_eta2_tau = np.exp(eta2_tilde * tau)
    exp_eta3_tau = np.exp(eta3_tilde * tau)
    exp_eta4_tau = np.exp(eta3_tilde * tau)
    S = seasonal_effect(deltas, T)
    kappa = kappa1 * exp_eta1_tau + S * exp_eta2_tau
    omega = omega1 * exp_eta2_tau
    rho = rho1 * exp_eta3_tau
    nu = nu1 * exp_eta4_tau
    return carr_wu_iv(kappa, omega, nu, rho, gamma, x, tau)


@jit(nopython=True, cache=True)
def jac_carr_wu_disprop_seasonal_fxd(Zf, f, FIXED, betas, p, k):
    x, tau, T, Z = FIXED
    kappa1, omega1_star, nu_star, rho_star = f.flatten()
    omega1, nu, rho, gamma = map_moments(omega1_star, nu_star, rho_star)
    eta1, eta2 = np.exp(betas.flatten()[:2])
    eta1_tau = np.exp(-eta1*tau)
    eta2_tau = np.exp(-eta2*tau)
    S = seasonal_effect(betas[2:], T)
    kappa = kappa1 * eta1_tau + S * eta2_tau
    omega = omega1 * eta1_tau
    ...
    z1 = z_kappa * eta1_tau
    z2 = z_omega * eta1_tau * omega1_star * omega
    z3 = z_nu * nu_star * nu
...
class CarrWuDisProp2SeasonalCorrNu:
    def __init__(self, mean:str = 'carr_wu_disprop2_s1_corr_nu'):
        self.mean = mean
        self.Zf = carr_wu_disprop2_seasonal_corr_nu
        self.J = None #jac_carr_wu_disprop_seasonal_fxd
        self._dict_means = dict({'carr_wu_disprop2_s1_corr_nu':2, 'carr_wu_disprop2_s2_corr_nu':4,
                                 'carr_wu_disprop2_s3_corr_nu':6, 'carr_wu_disprop2_s4_corr_nu':8})
        self.k_fxd = 4 + 4 + self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
...
# cwsf = CarrWuDisProportionalSeasonalFixed()
# cwsf._test_jac()
