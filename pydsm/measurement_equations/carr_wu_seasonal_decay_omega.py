...
def carr_wu_seasonal_decay_omega(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    eta1_star, eta2_star, kappa1, omega1_star, nu_star, rho_star = f.flatten()[:6]
    deltas = f[6:]
    ...
    omega = omega1 * np.exp(eta1_star * tau)
...
class CarrWuSeasonalDecayOmega:
    def __init__(self, mean:str = 'carr_wu_s1_decay_omega'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_decay_omega
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay_omega':2, 'carr_wu_s2_decay_omega':4,
                                 'carr_wu_s3_decay_omega':6, 'carr_wu_s4_decay_omega':8})
