...
def carr_wu_seasonal_decay_kappa_omega(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega1_star, nu_star, rho_star, eta1_star, eta2_star, eta3_star = f.flatten()[:7]
    deltas = f[7:]
    omega1, nu, rho, gamma = map_moments(omega1_star, nu_star, rho_star)
    # eta1 = np.exp(eta1_star)
    # eta2 = np.exp(eta2_star)
    # eta3 = np.exp(eta3_star)
    S = seasonal_effect(deltas, T)
    kappa = kappa1 * np.exp(eta1_star * tau) + S * np.exp(eta2_star * tau)
    omega = omega1 * np.exp((eta2_star + eta3_star) * tau)
...
class CarrWuSeasonalDecayKappaOmega:
    def __init__(self, mean:str = 'carr_wu_s1_decay_kappa_omega'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_decay_kappa_omega
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay_kappa_omega':2, 'carr_wu_s2_decay_kappa_omega':4,
                                 'carr_wu_s3_decay_kappa_omega':6, 'carr_wu_s4_decay_kappa_omega':8})
        self.k_fxd = 7 + self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
