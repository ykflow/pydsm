...
def carr_wu_seasonal_decay_kappa_rho(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega_star, nu_star, rho1_star, eta1_star, eta2_star, eta3_star = f.flatten()[:7]
    deltas = f[7:]
...
class CarrWuSeasonalDecayKappaRho:
    def __init__(self, mean:str = 'carr_wu_s1_decay_kappa_rho'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_decay_kappa_rho
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay_kappa_rho':2, 'carr_wu_s2_decay_kappa_rho':4,
                                 'carr_wu_s3_decay_kappa_rho':6, 'carr_wu_s4_decay_kappa_rho':8})
