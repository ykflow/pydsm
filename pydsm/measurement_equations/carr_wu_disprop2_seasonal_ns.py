...
def carr_wu_disprop2_seasonal(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa0, kappa1, omega1_star, nu_star, rho_star, eta1_tilde, eta2_tilde = f.flatten()[:7]
    omega1, nu, rho, gamma = map_moments(omega1_star, nu_star, rho_star)
    deltas = f[7:]
    slope1 = (1 - np.exp(-eta1_tilde * tau)) / (eta1_tilde*tau)
    slope2 = (1 - np.exp(-eta2_tilde * tau)) / (eta2_tilde * tau)
    S = seasonal_effect(deltas, T)
    kappa = kappa0 + kappa1 * slope1 + S * slope2
    omega = omega1 * slope2
...
class CarrWuDisProp2Seasonal:
    def __init__(self, mean:str = 'carr_wu_disprop2_s1'):
        self.mean = mean
        self.Zf = carr_wu_disprop2_seasonal
        self.J = None #jac_carr_wu_disprop_seasonal_fxd
        self._dict_means = dict({'carr_wu_disprop2_s1':2, 'carr_wu_disprop2_s2':4,
                                 'carr_wu_disprop2_s3':6, 'carr_wu_disprop2_s4':8})
        self.k_fxd = 4 + 2 + self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
