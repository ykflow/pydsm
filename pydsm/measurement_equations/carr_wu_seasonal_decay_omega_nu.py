...
def carr_wu_seasonal_decay_omega_nu(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega1_star, nu1_star, rho_star, eta1_star, eta2_star, eta3_star = f.flatten()[:7]
    deltas = f[7:]
...
class CarrWuSeasonalDecayOmegaNu:
    def __init__(self, mean:str = 'carr_wu_s1_decay_omega_nu'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_decay_omega_nu
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay_omega_nu':2, 'carr_wu_s2_decay_omega_nu':4,
                                 'carr_wu_s3_decay_omega_nu':6, 'carr_wu_s4_decay_omega_nu':8})
