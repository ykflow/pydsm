...
def carr_wu_seasonal_decay_kappa_omega_rho_nu(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega1_star, nu1_star, rho1_star, eta1_star, eta2_star, eta3_star, eta4_star, eta5_star = f.flatten()[:9]
    deltas = f[9:]
    omega1, nu1, rho1, gamma = map_moments(omega1_star, nu1_star, rho1_star)
    S = seasonal_effect(deltas, T)
    kappa = kappa1 * np.exp(eta1_star * tau) + S * np.exp(eta2_star * tau)
    omega = omega1 * np.exp(eta3_star * tau)
    rho = rho1 * np.exp(eta4_star * tau)
    nu = nu1 * np.exp(eta5_star * tau)
...
class CarrWuSeasonalDecayKappaOmegaRhoNu:
    def __init__(self, mean:str = 'carr_wu_s1_decay_kappa_omega_rho_nu'):
        self.mean = mean
        self.Zf = carr_wu_seasonal_decay_kappa_omega_rho_nu
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay_kappa_omega_rho_nu':2, 'carr_wu_s2_decay_kappa_omega_rho_nu':4,
                                 'carr_wu_s3_decay_kappa_omega_rho_nu':6, 'carr_wu_s4_decay_kappa_omega_rho_nu':8})
        self.k_fxd = 9 + self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
