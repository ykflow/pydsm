...
def carr_wu_prop_seasonal(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
    kappa1, omega1_star, nu_star, rho_star, eta_tilde = f.flatten()[:5]
...
class CarrWuPropSeasonal:
    def __init__(self, mean:str = 'carr_wu_prop_s1'):
        self.mean = mean
        self.Zf = carr_wu_prop_seasonal
        self.J = None #jac_carr_wu_disprop_seasonal_fxd
        self._dict_means = dict({'carr_wu_prop_s1':2, 'carr_wu_prop_s2':4,
                                 'carr_wu_prop_s3':6, 'carr_wu_prop_s4':8})
        self.k_fxd = 4 + 1 + self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
