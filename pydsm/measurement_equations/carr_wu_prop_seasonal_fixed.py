...
def carr_wu_prop_seasonal_fxd(f, FIXED, betas, p, m):
    x, tau, T, Z = FIXED
...
class CarrWuPropSeasonalFixed:
    def __init__(self, mean: str = 'carr_wu_prop_s1_fixed'):
        self.mean = mean
        self.Zf = carr_wu_prop_seasonal_fxd
        # self.J = jac_carr_wu_prop_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_prop_s1_fixed': 2, 'carr_wu_prop_s2_fixed': 4,
                                 'carr_wu_prop_s3_fixed': 6, 'carr_wu_prop_s4_fixed': 8})
        self.k_fxd = 4
        self.k_free = 0
        self.k_betas_free = self._dict_means[self.mean] + 1
        self.k_betas_pos = 0
        self._test_jac()
