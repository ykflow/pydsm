...
def carr_wu_custom(f, FIXED, betas, p, m):
    # x, tau, T, Z = FIXED
    # kappa1, omega_star, nu_star, rho_star, eta1_star, eta2_star, eta3_star, eta4_star = f.flatten()[:8]
    # delta1, delta2 = f.flatten()[8:]
    # omega, nu, rho, gamma = map_moments(omega_star, nu_star, rho_star)
    # S = delta1 * np.sin(2*np.pi * tau + eta1_star) + delta2 * np.cos(2*np.pi * tau + eta2_star)
    # kappa = kappa1 + S * np.exp(eta4_star * tau)
    # rho = rho * np.exp(eta3_star * tau)

    x, tau, T, Z = FIXED
    kappa0, kappa1, omega1_star, nu_star, rho_star, eta1_tilde, eta2_tilde, eta3_tilde  = f.flatten()[:8]
    omega1, nu, rho, gamma = map_moments(omega1_star, nu_star, rho_star)
    deltas = f[8:]
    # slope1 = np.exp(eta1_tilde * tau)
    slope2 = np.exp(eta2_tilde * tau*2)
    slope3 = np.exp(eta3_tilde * tau)
    S = seasonal_effect(deltas, T)
    kappa = kappa0*tau**eta1_tilde + kappa1 * slope2 + S * slope3
    omega = omega1 *tau**eta1_tilde
...
class CarrWuCustom:
    def __init__(self, mean:str = 'custom'):
        self.mean = mean
        self.Zf = carr_wu_custom
        # self.J = jac_carr_wu_seasonal_fxd
        self.J = None
        self._dict_means = dict({'carr_wu_s1_decay_kappa':2, 'carr_wu_s2_decay_kappa':4,
                                 'carr_wu_s3_decay_kappa':6, 'carr_wu_s4_decay_kappa':8})
        self.k_fxd = 8 + 8 #+ self._dict_means[self.mean]
        self.k_free = 0
        self.k_betas_free = 0
        self.k_betas_pos = 0
        self._test_jac()
