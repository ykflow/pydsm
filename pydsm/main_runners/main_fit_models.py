...
for i in range(M):
    ax[1].plot(timeline[:in_sample], profile_a[:, i], color=cm(np.abs(profile_grid_omega[i]/21)), label='_nolegend_')
ax[1].plot(timeline[:in_sample], f_nls[:in_sample, 6], color='black',  label='NRLS')
ax[0].set_xlabel(r'$\log \sigma^2_{\eta_3}$', size=30)
ax[0].set_ylabel(r'Log-Lik', size=30)
ax[1].legend()
ax[0].legend()
ax[0].set_xlim(-20, -5)
ax[1].set_xlim(timeline[:in_sample].min(), timeline[:in_sample].max())
ax[0].set_title(r'Profile Likelihood as a function of $\log \sigma^2_{\eta_3}$', size=35)
ax[1].set_title(r'Filtered paths for $\eta_{3,t}$ for various $\log \sigma^2_{\eta_3}$', size=35)
plt.tight_layout()
# plt.show()
plt.savefig(os.path.join(plotfolder, 'profile_eta3.pdf'))
#
