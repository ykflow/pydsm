...
def collapsed_extended_kalman_filter(yx, Zf, FIXEDx, betas, c, Phi, Q, H, a1, P1, N, k, Wx, Ix, Sq=None, Iq=None, Jac=None):
    ...
    for t in range(N):
        ...
        H_star[t] = pinv(tmp0 @ Jx)
        W_star = H_star[t] @ tmp0
        y_star[t] = a[t] + H_star[t] @ tmp0 @ (yx[t] - Zfx)

        # Prediction error decomposition
        vx = (y_star[t] - a[t]) * Ix[t]
        epsx = yx[t] - Zfx - Jx @ vx
        # epsx = (np.eye(px) - Jx @ W_star) @ (yx[t] - Zfx + Jx @ a[t])
        Fx = P[t] + H_star[t]
        invFx = inv(Fx)
        tmp1 = P[t] @ invFx
        ...
        LL[t] = (ll_f + ll_y + 0.5 * logdet(H_star[t])) * Ix[t]
    ...
    return a[:N], P[:N], a_t[:N], P_t[:N], LL[:N], y_star[:N], H_star[:N]
...
#         FIXEDx.append(t)
#
# results = collapsed_extended_kalman_filter(yx, Zf, FIXEDx, np.array([]),
#                                            # c, T,
#                                            Q, H, a1, P1, n, m, Wx, Ix)
# print(results[-1].sum())
#
